...
```mermaid
graph TB
    subgraph "bigdata-net (Docker Network)"
        ZK["🦁 Zookeeper<br/>:2181"]
        NN["🗂️ NameNode<br/>:9870"]
        DN["🗂️ DataNode"]
        ...
        HBM["📊 HBase Master<br/>:16010"]
        HBR["📊 HBase RegionServer"]

        HIVE["🐝 Hive Server<br/>:10000"]
        HMETA["🐝 Hive Metastore"]
        PG[("🐘 PostgreSQL<br/>(Metastore)")]

        SM["⚡ Spark Master<br/>:8080"]
        SW["⚡ Spark Worker<br/>:8081"]

        TRINO["🚀 Trino<br/>:8090"]

        CASS["🔵 Cassandra<br/>:9042<br/>(Standalone)"]
        NEO["🕸️ Neo4j<br/>:7474<br/>(Standalone)"]

        KAFKA["📨 Kafka<br/>:9092"]
        KC["📅 Kafka Connect<br/>:8083"]

        JUP["📓 Jupyter<br/>:8888"]
        HUE["🎨 Hue (Web UI)<br/>:8889"]
        AF["🌬️ Airflow<br/>:8085"]
    end
...
```mermaid
graph LR
    subgraph "Core Storage & Coordination"
    ...
    subgraph "Standalone Services"
        CASS["🔵 Cassandra<br/>(No dependencies)"]
        NEO["🕸️ Neo4j<br/>(No dependencies)"]
        AF["🌬️ Airflow<br/>(No dependencies)"]
    end
